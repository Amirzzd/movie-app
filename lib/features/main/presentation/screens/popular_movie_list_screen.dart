...
class PopularMovieListScreen extends StatefulWidget {
  static const routeName = '/popular';

  const PopularMovieListScreen({super.key});

  @override
  State<PopularMovieListScreen> createState() => _PopularMovieListScreen();
}

class _PopularMovieListScreen extends State<PopularMovieListScreen> {
  ...
  void initState() {
    ...
    bloc.add(LoadPopularMovieListEvent(page: page,));
  }
  @override
  void dispose() {
    super.dispose();
  ...
  void paginate(BuildContext context){
    scrollController.addListener((){
      if(scrollController.position.atEdge && scrollController.position.pixels != 0 && bloc.hasReachMax == false ){
          bloc.add(LoadPopularMovieListEvent(
              page: ++page,
           ));
         }
       }
